...
- **You cannot kill those processes from here, so do not start one you are not going to need.** Anything launched through the app is owned by its sandbox: `Stop-Process` and `taskkill /F` both return *Access is denied*, with the sandbox disabled too, and so does the user's own non-elevated shell. Locate it with `Get-NetTCPConnection -LocalPort 5001 -State Listen` and hand the user the elevated command from `TESTING.md` rather than retrying. An orphan spends nothing while `sockets` is 0 — `judged` unchanged across two `/health` checks is the proof, and is what to report.
